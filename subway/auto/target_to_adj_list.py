...
def target_to_adj(PIK, tgtFile, mol_jsonFile, rxn_jsonFile):
    ...
    with open(tgtFile, encoding='utf-8') as csvf2:
        ...
        for rows in csvReader2:
            ...
            # Step 1
            a_m_node = next((item for item in csvReader if item["SMILES"] == a_SMILES), None)
            a_mol_id = a_m_node["id"]
            b_m_node = next((item for item in csvReader if item["SMILES"] == b_SMILES), None)
            b_mol_id = b_m_node["id"]
            ab_m_node = next((item for item in csvReader if item["SMILES"] == ab_SMILES), None)
            ...
            ab_m_node = next((item for item in csvReader if item["SMILES"] == ab_SMILES), None)
            ab_mol_id = ab_m_node["id"]
            c_m_node = next((item for item in csvReader if item["SMILES"] == c_SMILES), None)
            c_mol_id = c_m_node["id"]
            F_m_node = next((item for item in csvReader if item["SMILES"] == F_SMILES), None)
            ...
            F_m_node = next((item for item in csvReader if item["SMILES"] == F_SMILES), None)
            F_mol_id = F_m_node["id"]
            carb_node = next((item for item in csvReader if item["SMILES"] == carb_SMILES), None)
